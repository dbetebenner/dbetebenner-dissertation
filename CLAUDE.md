...
- **"Edit chapter X"** — R-package layout: `packages/r-packages/{{rPackage.name}}/ui/www/chapters/X.qmd`; content-only layout: `thesis/chapters/X.qmd`. Either way, `build-thesis.yml` rebuilds the PDF on push (the R package's for the former, this repo's for the latter).
