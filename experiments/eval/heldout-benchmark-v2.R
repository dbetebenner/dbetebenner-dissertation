# FROZEN EVALUATION SCRIPT v2 (charter 11.2: frozen surfaces).
# Registered experiments list this file in frozen_files; once a run is
# `planned` or `running`, edits here invalidate the run.
#
# v2 = v1 (frozen with exp_2026_08_02_001..003, untouched) plus one model
# mapping: `irtc_native_1f_<family>` fits the one-factor copula with the
# NATIVE irtc engine (fit_copula_1f(engine = "irtc") -- person-wise
# marginal likelihood, no O(2^J) joint table; ledger C0006). Everything
# else -- integrity gates, folds, metrics, no-verdict rule -- is identical.
#
# Usage: Rscript experiments/eval/heldout-benchmark-v2.R <run_id>
#
# Contract:
...
fit_model <- function(model_id) {
  ...
  } else if (grepl("^irtc_native_1f_", model_id)) {
    fam <- sub("^irtc_native_1f_", "", model_id)
    fit <- fit_copula_1f(train_cc, family = fam, nq = 25, engine = "irtc")
...
jsonlite::write_json(
  list(
    ...
    eval_script = "experiments/eval/heldout-benchmark-v2.R",
